...
, (SUM(CONVERT(int,vac.new_vaccinations)) OVER (Partition by dea.location Order by dea.Date)/population)*100 as Percent_RollingPeopleVaccinated
From [covid_db]..[CovidDeaths] dea
...
as(
Select dea.continent, dea.location, dea.date, dea.population, vac.new_vaccinations
, SUM(CONVERT(int,vac.new_vaccinations)) OVER (Partition by dea.Location Order by dea.Date) as RollingPeopleVaccinated --, (RollingPeopleVaccinated/population)*100
From [covid_db]..[CovidDeaths] dea
Join [covid_db]..[CovidVaccinations] vac
	On dea.location = vac.location
	and dea.date = vac.date
...
Select *, (RollingPeopleVaccinated/Population)*100 as percent_dt
From PopvsVac
